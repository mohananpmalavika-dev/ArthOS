...
const LongitudinalLearningDashboard = ({ userId }) => {
  ...
  const loadLongitudinalData = async () => {
    setLoading(true);
    try {
      const [lifecycleRes, patternsRes, trendsRes, insightsRes, anomaliesRes, journalRes] = await Promise.all([
        ...
        fetch(`/api/longitudinal/anomalies?userId=${userId}&acknowledged=false`).then(r => r.json()),
        fetch(`/api/longitudinal/journal?userId=${userId}&limit=10`).then(r => r.json())
      ]);

      if (lifecycleRes.success) setLifecycle(lifecycleRes.lifecycle || lifecycleRes);
      ...
      if (journalRes.success) setJournal(journalRes.entries || []);
    } catch (error) {
      ...
      <div className="max-w-7xl mx-auto mb-8">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-600">Track your evolution and discover patterns in your financial behavior</p>
          </div>
          ...
          <div className="bg-white rounded-lg shadow-lg p-8 border-l-4 border-indigo-600">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
              <div className="bg-indigo-50 rounded-lg p-4">
                ...
                <div className="flex items-end gap-2">
                  <p className="text-3xl font-bold text-indigo-600">{lifecycle.financial_maturity_score || 0}</p>
            ...
            <div className="flex items-center justify-between pt-4 border-t border-gray-200">
              ...
              <div>
                ...
                <p className="text-lg font-semibold text-gray-900">{lifecycle.months_on_platform || 0}</p>
              </div>
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    {insights.slice(0, 3).map(insight => (
                      <div key={insight.id} className="bg-amber-50 border-l-4 border-amber-400 p-4 rounded">
                        <div className="flex items-start justify-between">
                          <div>
                            <p className="font-semibold text-gray-900">{insight.insight_title}</p>
                            <p className="text-sm text-gray-600 mt-1">{insight.insight_description}</p>
                          </div>
              ...
              {patterns.length === 0 ? (
                ...
                <div className="space-y-3">
                  {patterns.map(pattern => (
                    <div key={pattern.id} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-4 text-sm">
                        {pattern.avg_amount && (
                          <div className="bg-gray-50 rounded p-2">
                            ...
                            <p className="font-semibold">₹{Math.round(pattern.avg_amount).toLocaleString()}</p>
                          </div>
                        )}
                        <div className="bg-gray-50 rounded p-2">
                        ...
                        <div className="bg-gray-50 rounded p-2">
                          ...
                          <p className="font-semibold capitalize text-indigo-600">{pattern.pattern_strength}</p>
                        </div>
                      </div>
                    </div>
                  ))}
          ...
          {activeTab === 'trends' && (
            <div className="p-6">
              ...
              {trends.length === 0 ? (
                ...
                <div className="space-y-3">
                  {trends.map(trend => (
                    <div key={trend.id} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition">
                      <div className="flex items-start justify-between mb-3">
                        ...
                        <div className="text-right">
                          <p className={`text-lg font-semibold ${
                            ...
                            {trend.percent_change_12m > 0 ? '+' : ''}{trend.percent_change_12m?.toFixed(1) || '0'}%
                          </p>
                      ...
                      <div className="grid grid-cols-3 gap-3 text-sm">
                        <div className="bg-gray-50 rounded p-2">
                          ...
                          <p className="font-semibold">₹{Math.round(trend.current_value).toLocaleString()}</p>
                        </div>
                        <div className="bg-gray-50 rounded p-2">
                          <p className="text-gray-600 text-xs">Average</p>
                          <p className="font-semibold">₹{Math.round(trend.average_value_12m || 0).toLocaleString()}</p>
                        </div>
                        <div className="bg-gray-50 rounded p-2">
                          <p className="text-gray-600 text-xs">Confidence</p>
                          <p className="font-semibold">{Math.round(trend.trend_confidence_score)}%</p>
                        </div>
                      </div>
                    </div>
                  ))}
            ...
            <div className="p-6">
              ...
              {journal.length === 0 ? (
                <div className="text-center py-12 text-gray-500">
                  ...
                  <p>Your financial journey begins here. Start by recording your first milestone!</p>
                </div>
              ) : (
                      ...
                      <div className="flex gap-4">
                        <div className="flex-shrink-0">
                          <div className="flex items-center justify-center h-12 w-12 rounded-full bg-indigo-600">
                            {entry.entry_type === 'achievement' && <CheckCircle2 size={24} className="text-white" />}
                            {entry.entry_type === 'milestone' && <Target size={24} className="text-white" />}
                            {entry.entry_type === 'challenge' && <AlertCircle size={24} className="text-white" />}
                            {entry.entry_type === 'learning' && <Lightbulb size={24} className="text-white" />}
                            {entry.entry_type === 'decision' && <Zap size={24} className="text-white" />}
                          </div>
                        </div>
                          ...
                          <div className="flex items-start justify-between mb-2">
                            <div>
                              ...
                              <p className="flex items-center gap-1 text-sm text-gray-600 mt-1">
                                <Calendar size={14} /> {new Date(entry.journal_date).toLocaleDateString()}
                              </p>
                            </div>
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-gray-600 mb-4">
                  We've noticed {anomalies.length} unusual pattern(s) in your financial behavior. Review them to ensure everything looks correct.
                </p>
                <div className="flex gap-3 flex-wrap">
                  {anomalies.slice(0, 2).map(anomaly => (
                    <span key={anomaly.id} className={`px-3 py-1 rounded text-xs font-semibold ${
                      anomaly.severity_level === 'critical' ? 'bg-red-100 text-red-700' :
                      anomaly.severity_level === 'high' ? 'bg-orange-100 text-orange-700' :
