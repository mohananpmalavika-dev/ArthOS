...
export function generatePersonalizedInsights(assessmentResult, assessment) {
  ...
  if (behaviourScore < 25) {
    insights.push({
      ...
      actionable:
        "Pick ONE trigger (social spending, bored shopping, stress relief) and replace it this week.",
    ...
    insights.push({
      ...
      insight:
        "Some days you're disciplined. Other days, not so much. This inconsistency is expensive.",
  ...
  if (awarenessScore < 10) {
    insights.push({
      ...
      actionable:
        "List your monthly expenses from memory. Then track for 7 days. Compare. This gap is your blind spot.",
    ...
    insights.push({
      ...
      actionable:
        "Spend 30 minutes this week getting exact numbers for: total debt, fixed expenses, and savings.",
      signal: `Awareness Score: ${Math.round(awarenessScore)}/30`
    });
  } else {
    insights.push({
  ...
  if (survivalMonthsRaw < 1) {
    insights.push({
      ...
      actionable:
        "Make emergency savings your ONLY financial goal for the next 60 days. Target: 1 month of expenses.",
  ...
  if (savingsRate < 0) {
    insights.push({
      ...
      insight:
        "You're spending more than you earn. This is burning your savings by ₹" +
        Math.round(Math.abs(monthlyIncome - monthlyExpenses)) +
        " per month.",
    ...
    insights.push({
      ...
      actionable:
        "Find ₹2-5K per month to save. Cut one discretionary category (delivery, subscriptions, etc).",
  ...
  const personalityInsights = {
    ...
    Survivor: {
      ...
      actionable:
        "Allocate 5-10% of savings to a small growth opportunity (investment, skill, side income)."
    },
    Optimizer: {
    ...
    "Risk Taker": {
      ...
      actionable:
        "Before any major decision, sleep on it. If it's good, it'll still be good tomorrow."
    }
  };
...
export function detectBehaviouralPatterns(assessment, historicalData) {
  ...
  if (
    behaviour.unplannedPurchaseFreq === "very_frequently" ||
    behaviour.impulseWaitRule === "never"
  ) {
    patterns.push({
      id: "impulse_override",
