...
const BADGE_DEFINITIONS = [
  // --- Score milestones ---
  {
    id: "first_assessment",
    label: "First Steps",
    description: "Completed your first financial health assessment",
    icon: "🌟",
    category: "score"
  },
  {
    id: "score_30",
    label: "On Your Way",
    description: "Reached a financial health score of 30+",
    icon: "📈",
    category: "score",
    check: s => s >= 30
  },
  {
    id: "score_50",
    label: "Halfway There",
    description: "Reached a financial health score of 50+",
    icon: "🔥",
    category: "score",
    check: s => s >= 50
  },
  {
    id: "score_70",
    label: "Resilient",
    description: "Reached a financial health score of 70+",
    icon: "💪",
    category: "score",
    check: s => s >= 70
  },
  {
    id: "score_90",
    label: "Sovereign",
    description: "Reached a financial health score of 90+",
    icon: "👑",
    category: "score",
    check: s => s >= 90
  },
  {
    id: "score_100",
    label: "Perfect Score",
    description: "Achieved a perfect financial health score",
    icon: "🏆",
    category: "score",
    check: s => s >= 100
  },
  ...
  // --- Improvement milestones ---
  {
    id: "improve_10",
    label: "Rising Star",
    description: "Improved your score by 10+ points from first assessment",
    icon: "⭐",
  ...
  },
  {
    ...
    description: "Improved your score by 25+ points from first assessment",
    icon: "⭐",
    category: "improvement",
    check: d => d >= 25
  },
  {
    ...
    description: "Improved your score by 50+ points from first assessment",
    icon: "🌈",
    category: "improvement",
    check: d => d >= 50
  },
  ...
  // --- Decision tracking ---
  {
    ...
    description: "Tracked 5 financial decisions",
    icon: "🎯",
    category: "decisions",
    check: c => c >= 5
  },
  {
    ...
    description: "Tracked 25 financial decisions",
    icon: "🎯",
    category: "decisions",
    check: c => c >= 25
  },
  ...
  // --- Special ---
  {
    ...
    description: "Enriched your assessment with banking data",
    icon: "🔍",
    category: "special"
  },
  {
    id: "twin_simulated",
    label: "Future Gazer",
    description: "Simulated your financial twin scenarios",
    icon: "🔮",
    category: "special"
  },
  {
    ...
    description: "Connected to the ARTH.OS partner ecosystem",
    icon: "🔗",
    category: "special"
  }
];
...
export function getUnlockedBadges() {
  ...
  return ids.map(id => BADGE_DEFINITIONS.find(b => b.id === id)).filter(Boolean);
}
...
export function checkAndUnlockMilestones({
  ...
  for (const badge of BADGE_DEFINITIONS) {
    if (loadMilestones().includes(badge.id)) {
      continue;
    }
    ...
    switch (badge.category) {
      case "score":
        if (badge.check && currentScore != null) {
          shouldUnlock = badge.check(currentScore);
        }
        // first_assessment is special — auto-unlock on first score
        if (badge.id === "first_assessment" && currentScore > 0 && assessmentCount >= 1) {
          shouldUnlock = true;
        }
        break;
      case "streak":
        if (badge.check) {
          shouldUnlock = badge.check(streak);
        }
        break;
      case "improvement":
        if (badge.check) {
          shouldUnlock = badge.check(improvement);
        }
        break;
      case "count":
        if (badge.check) {
          shouldUnlock = badge.check(assessmentCount);
        }
        break;
      case "decisions":
        if (badge.check) {
          shouldUnlock = badge.check(decisionCount);
        }
        break;
      case "special":
        if (badge.id === "sms_enrich" && hasSmsEnrichment) {
          shouldUnlock = true;
        }
        if (badge.id === "twin_simulated" && hasTwinSimulation) {
          shouldUnlock = true;
        }
        if (badge.id === "partner_connected" && hasPartnerConnection) {
          shouldUnlock = true;
        }
        break;
    }
