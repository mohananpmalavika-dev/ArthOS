...
export function predictImpulseSpendingRisk(assessment, result, historicalData = []) {
  ...
  if (historicalData.length === 0) {
    ...
    const risk = (1 - (awareness / 30)) * 0.4 + (1 - (behaviour / 45)) * 0.4 + (1 - (stability / 25)) * 0.2;
    return {
...
export function predictStressSpending(assessment, result) {
  ...
  let baseProbability = stressSpendTrigger ? 0.7 : 0.2;
...
export function predictArchetypeEvolution(assessment, result, previousAssessments = []) {
  ...
  if (previousAssessments.length > 0) {
    ...
    const awarenessGrowth = (result?.awarenessScore || 0) - (previousAssessments[0]?.awarenessScore || 0);
...
export function trainBehaviorModels(historicalDataset) {
  ...
  const savingsLabels = historicalDataset.map(record =>
    ((record.result?.stabilityScore || 0) / 25) * 100
  );
...
export function generateBehaviorPredictionReport(assessment, result, historicalData = []) {
  return {
    ...
    modelConfidence: historicalData.length > 50 ? 'High' : historicalData.length > 10 ? 'Medium' : 'Low',
    recommendedActions: [
