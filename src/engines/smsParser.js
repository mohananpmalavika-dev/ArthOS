...
const AMOUNT_PATTERN =
  /(?:₹|Rs|rs|INR|amount|debited|credited|spent|transferred|paid|charged|reversed)\s*[.:]*\s*(\d+(?:[,.\s]\d{1,3})*(?:\.\d{2})?)/i;
...
export function mapSignalsToBehaviour(signals) {
  ...
  const behaviourMapping = {
    ...
    regretImpulseFreq:
      signals.spendWhenBored === "very_likely"
        ? "often"
        : signals.spendWhenBored === "sometimes"
          ? "sometimes"
          : "rarely",
...
export function mapSignalsToStability(signals) {
  ...
  const bufferDrainRisk =
    signals.smallAmountFreq > 0.6 ? "high" : signals.smallAmountFreq > 0.3 ? "medium" : "low";
...
export function generateSMSIngestPrompt() {
  return {
    ...
    privacyNote:
      "Your SMS data is processed locally. No messages are stored. Only aggregated signals are saved."
  };
}
